...
Item {
    ...
    // no need to avoid the whole cutout area in the middle, no content exactly there
    property int _topMargin: orientation == Orientation.Portrait
                             ? Math.max(0, Screen.topCutout.height - Theme.paddingMedium) : 0

    signal dateClicked()

    height: _topMargin + header.height
