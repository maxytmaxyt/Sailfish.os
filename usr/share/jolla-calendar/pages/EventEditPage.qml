...
Dialog {
        ...
        Column {
            ...
            ValueButton {
                ...
                value: {
                    ...
                    case Qt.LocalTime:
                        //% "None"
                        return qsTrId("calendar-me-clock_time")
                    case Qt.UTC:
                        //% "Coordinated universal time"
                        return qsTrId("calendar-me-utc")
                    case Qt.TimeZone:
                        //: %1 will be replaced by localized country and %2 with localized city
                        //% "%1, %2"
                        return qsTrId("calendar-me-localized-timezone").arg(localizer.country).arg(localizer.city)
                    }
                }
                onClicked: {
    ...
    onAccepted: {
        ...
        if (dialog._isEdit && modification.instanceId == "") {
            console.warn("Unable to dissociate event", dialog.event.instanceId,
                         "at date", dialog.occurrence.startTime)
            return
        }
