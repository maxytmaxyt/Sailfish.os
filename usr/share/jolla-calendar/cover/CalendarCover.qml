    ...
    Column {
        ...
        Item {
                ...
                function update() {
                    ...
                    for (var i = 0; i < allEvents.count; ++i) {
                        ...
                        if (event.allDay || now < occurrence.endTime) {
                            activeAndComing.append({ displayLabel: event.displayLabel,
                                                       allDay: event.allDay,
                                                       startTime: occurrence.startTime,
                                                       endTime: occurrence.endTime,
                                                       color: event.color,
                                                       cancelled: event.status == CalendarEvent.StatusCancelled })
            ...
            ListView {
                ...
                // showing as many as fits but at least 2 delegates
                property int delegateImplicitHeight: 2 * fontMetrics.height
                property int maxDelegateCount: Math.max(2, (parent.height + spacing) / (delegateImplicitHeight + spacing))
                property int eventHeight: Math.min((parent.height - Theme.paddingSmall - spacing) / 2,
                                                   delegateImplicitHeight)

                clip: true
                model: activeAndComing
                interactive: false
                width: parent.width
                height: maxDelegateCount * eventHeight + (spacing * (maxDelegateCount - 1))
                spacing: Theme.paddingSmall
                visible: count > 0
