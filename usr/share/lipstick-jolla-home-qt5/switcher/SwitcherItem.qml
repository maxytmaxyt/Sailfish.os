...
EditableGridDelegate {
    ...
    readonly property bool quickSwitchingApp: index == secondLastAppIndex
                                              && Lipstick.compositor.quickAppToggleGestureExceeded
    ...
    function applyVisibility() {
        if (cover && cover.surface) {
            ...
            window.userData.coverVisibility = (visible || activationTimer.running) ? QtQuick.Window.Minimized
                                                                                   : QtQuick.Window.Hidden
        }
    }
    ...
    // forces cover visible for small time to avoid temporary hidden state for window surface if it was shown as cover
    function triggerActivating() {
    ...
    Connections {
        ...
        onExposedChanged: {
            if (window.userData.exposed && activationTimer.running) {
    ...
    Timer {
        ...
        interval: 10000
        onTriggered: wrapper.applyVisibility()
    }
    ...
    Backgrounds.CoverBackground {
        ...
        visible: (cover
                 ...
                 || !running
                 || startupTimer.running
                 || coverOpacityAnimation.running
    ...
    WindowPixmapItem {
        ...
        yOffset: {
            ...
            // with display notch do a bit of offset to avoid showing likely less important content there
            if (Screen.topCutout.height > 0
                    && rotation == 0
                    && windowPixmap.windowId == wrapper.windowId && wrapper.window && wrapper.window.surface) {
                // offset behaves as relative to size
                return Screen.topCutout.height / Screen.height
            }
        ...
        rotation: windowPixmap.windowId == wrapper.windowId && wrapper.window && wrapper.window.surface
                          ? QtQuick.Screen.angleBetween(QtQuick.Screen.primaryOrientation,
                                                        wrapper.window.surface.contentOrientation)
                          : 0
    ...
    Rectangle {
        ...
        anchors.fill: parent
        opacity: ((currentApp && !Lipstick.compositor.homePeeking)
                  || quickSwitchingApp || windowMapPending || launching || wrapper.closing)
                 ? Theme.opacityLow
        ...
        ParallelAnimation {
            ...
            SequentialAnimation {
                ...
                ScriptAction {
                    script: {
                        if (window) {
                            Lipstick.compositor.windowsBeingClosed
                                    = Lipstick.compositor.windowsBeingClosed.concat([window.userData])
                            if (cover && cover.surface && cover.surface.windowProperties.CATEGORY == "silica-cover") {
                                ...
                                if (window.surface.visibility == QtQuick.Window.Hidden) {
                                    // closing hidden surface / QWindow can make the client side omit
                                    // quitOnLastWindowClosed handling. avoid such state.
                                    window.surface.visibility = QtQuick.Window.Minimized
                                }
