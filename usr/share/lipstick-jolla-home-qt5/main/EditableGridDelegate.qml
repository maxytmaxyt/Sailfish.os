...
Item {
    ...
    MouseArea {
            ...
            if (item && item !== wrapper) {
                if (!manager.supportsFolders) {
                    ...
                    if (offset < folderThreshold) {
                        if (Math.abs(index - item.modelIndex) > 1
                                || index > item.modelIndex
                                || item.y !== wrapper.targetY) {
                        ...
                        if (Math.abs(index - item.modelIndex) > 1
                            ...
                            manager.folderItem = null
                               ...
                               && manager.supportsFolders
                               && item.folderItemCount < 99
                               && !isFolder) {
        ...
        onPositionChanged: {
            if (!dragged && (Math.abs(_pressX - mouseX) > Theme.paddingLarge
                             || Math.abs(_pressY - mouseY) > Theme.paddingLarge)) {
                dragged = true
            }
        }
