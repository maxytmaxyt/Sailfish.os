// iPhone gesture mapping for app close:
//   Swipe UP from bottom (slow) → peek at home / begin close
//   Release past threshold     → app closes (returns to home)
//   Release before threshold   → snap back (cancel)
/*
    ...
    function close() {
        if (!delayClose && !resetAnimation.running && !peekFilter.active) {
            if (contentVisible) {
                ...
                closeAnimating = true
                aboutToClose()
                clipEndAnimation.duration = 350  // iPhone-style: faster close animation
                _startFade()
            } else {
                closing = false
                closed()
    ...
    PeekAreaFilter {
        ...
        onGestureCanceled: {
            ...
            if (peekFilter.closing) {
                ...
                clipEndAnimation.duration = 250  // iPhone-style: snappy cancel/reset
            } else {
                clipEndAnimation.duration = 180  // iPhone-style: fast snap back
        ...
        onGestureTriggered: {
            ...
            peekArea.closeAnimating = true
            // iPhone-style: quick completion like iOS spring animation
            clipEndAnimation.duration = 200 * (clipEndAnimation.to - peekFilter.absoluteProgress) / clipEndAnimation.to
            peekArea._startFade()
        }
        ...
        states: [
            State {
                ...
                PropertyChanges {
                    target: peekArea
                    // iPhone-style: smoother, more linear fade
                    opacity: (1 - Math.max(peekArea.peekFilter.progress - 0.1, 0) / 0.9)
                }
            }, State {
