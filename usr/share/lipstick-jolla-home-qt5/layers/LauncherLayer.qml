...
EdgeLayer {
            ...
            if (pinningHintCounter.active)
                if (_pinningHint) {
                    ...
                    if (hintComponent.status === Component.Ready) {
                        ...
                        _pinningHint.launcherAbsoluteExposure = Qt.binding(function() {
                            return launcherLayer.absoluteExposure
                        })
                        _pinningHint.closed.connect(pinningHintCounter.exhaust)
                    } else {
                        console.log("LauncherLayer failed create PinnedLauncherHint component",
                                    hintComponent.errorString())
                    }
                }
        } else {
            _wasPinning = true
    ...
    on_DragActiveChanged: {
        if (_dragActive) {
            ...
            var invertedOrientation = orientation === Qt.InvertedPortraitOrientation
    ...
    // iPhone-style: smaller hint (home indicator bar height)
    hintHeight: Theme.paddingLarge * 2
    hintDuration: 200  // iPhone-style: fast, snappy
    ...
    Timer {
        ...
        interval: 99999  // iPhone-style: pinning disabled (no long-press-to-pin)
        ...
        onRunningChanged: launcherLayer._startPinPosition = launcherLayer._transposed ? launcherLayer.x
                                                                                      : launcherLayer.y
        onTriggered: {
