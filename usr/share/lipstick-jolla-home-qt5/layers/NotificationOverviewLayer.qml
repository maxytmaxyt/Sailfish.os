...
Layer {
    ...
    readonly property bool lockScreenAnimated: lipstickSettings.blankingPolicy === "default"
                                               || !Lipstick.compositor.lockScreenLayer.showNotifications
    property bool previewExpanded
    property bool lockScreenLocked

    property real _launcherPosition: Math.max(Lipstick.compositor.launcherLayer.exposure,
                                              Lipstick.compositor.topMenuLayer.exposure)
    ...
    exposed: {
        ...
        return Lipstick.compositor.lockScreenLayer.exposed
                || Lipstick.compositor.homeVisible
                || lipstickSettings.lowPowerMode
    }
    ...
    property bool _opacityBehaviorEnabled: state !== "peeking_through_launcher"
                                           && state !== "switcher-menu"
    ...
    states: [
        ...
        State {
            ...
            when: Lipstick.compositor.homeLayer.currentItem == Lipstick.compositor.switcherLayer
                  && !lockScreenLocked
                  && ((Lipstick.compositor.peekingLayer.exposed && Lipstick.compositor.peekingAtHome)
                      || (Lipstick.compositor.launcherPeeking && !Lipstick.compositor.homeLayer.active))
        },
        State {
