...
StackLayer {
    ...
    onQueueWindow: {
        if (Desktop.startupWizardRunning) {
            if (JollaSystemInfo.matchingPidForCommand(window.window.processId,
                                                      '/usr/bin/jolla-startupwizard', true) !== -1
                        || JollaSystemInfo.matchingPidForCommand(window.window.processId,
                                                                 '/usr/bin/sailfish-captiveportal', true) !== -1) {
