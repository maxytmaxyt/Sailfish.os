...
Layer {
    ...
    function show(w, quick) {
        ...
        if (exclusive) {
            ...
            // If the layer is exclusive remove all other queued windows before queuing a new one
            var head
            while (queued.length) {
                head = queued.shift()
    ...
    function _showWindow(w, quick) {
        ...
        if (Lipstick.compositor.debug) {
            ...
            console.log("StackLayer: Are we locked: ", locked, "effective delay switch:", _effectiveDelaySwitch,
                        "delay switch:", delaySwitch)
