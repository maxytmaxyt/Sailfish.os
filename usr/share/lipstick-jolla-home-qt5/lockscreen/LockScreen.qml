...
ApplicationWindow {
    ...
    allowedOrientations: (Screen.sizeCategory > Screen.Medium || homescreenOrientationConfig.value)
    ...
    initialPage: Component {
        Page {
            ...
            readonly property bool interactionExpected: !visible
                                                        || Lipstick.compositor.lockScreenLayer.dimmer.relativeDim
                                                        || lockScreen.pinQueryPannable
                                                        || Lipstick.compositor.topMenuLayer.exposed
            ...
            StateGroup {
                states: State {
                    ...
                    PropertyChanges {
                        ...
                        showNotifications: (Desktop.deviceLockState == DeviceLock.Unlocked
                                            || Desktop.deviceLockState == DeviceLock.Locked)
                                           && Lipstick.compositor.lockScreenLayer.opaque
                                           && systemStarted.value
                                           && ((Lipstick.compositor.lockScreenLayer.vignette.opened
            ...
            Pannable {
                ...
                readonly property bool locked: lipstickSettings.lockscreenVisible
                                               || Desktop.deviceLockState >= DeviceLock.Locked
                ...
                // If startup wizard is running it will show the pin query.
                // Show only when device started and sim pin is required.
                readonly property bool needPinQuery: !Desktop.startupWizardRunning
                                                     && PinQueryAgent.simPinRequired
                                                     && (!PinQueryAgent.simPinCompleted || showPinQuery)
                ...
                function unlock(playTone) {
                    ...
                    // Lipstick.compositor.unlocked signal is emitted when we can unlock
                    // and showPinQuery cannot block unlocking from this point onwards.
                    lockScreen.showPinQuery = false
                    if (lockScreen.pinQueryPannable) {
                        lockScreen.pinQueryPannable.deviceWasLocked = false
                    }

                    if ((lockContainer.x > 0 || absoluteProgress > 0)
                            && !Lipstick.compositor.lockScreenLayer.lockScreenEventsEnabled
                            && (Lipstick.compositor.notificationOverviewLayer.hasNotifications
                                || Desktop.settings.left_peek_to_events)
                ...
                peekFilter {
                    enabled: !lockScreen.lowPowerMode
                             && !Lipstick.compositor.notificationOverviewLayer.revealingEventsView
                             && lockScreen.locked && !Desktop.startupWizardRunning
                ...
                pannableItems: [
                    PannableItem {
                        ...
                        readonly property Item nextItem: deviceLockItem.locked ? deviceLockContainer
                                                                               : lockScreen.pinQueryPannable
