...
Row {
    ...
    property bool registered: _networkRegistrationStatus === "registered"
                              || _networkRegistrationStatus === "roaming"
    property bool textVisible: cellularNetworkNameStatusIndicatorHome.text.length
                               || cellularNetworkNameStatusIndicatorVisitor.text.length
    ...
    property real _separatorWidth: cellularNetworkNameStatusSeparator.width + (_showHome ? Theme.paddingSmall : 0)
                                   + (_showVisitor ? Theme.paddingSmall : 0)
    property real _maxTextWidth: maxWidth - (_showSeparator ? _separatorWidth : 0)
    property real _maxHomeTextWidth: _maxTextWidth
                                     - (_showVisitor
                                        ? Math.min(cellularNetworkNameStatusIndicatorVisitor.implicitWidth,
                                                   (_maxTextWidth / 2))
                                        : 0)
    property real _maxVisitorTextWidth: _maxTextWidth
                                        - (_showHome ? Math.min(cellularNetworkNameStatusIndicatorHome.implicitWidth,
                                                                (_maxTextWidth / 2))
                                                     : 0)
