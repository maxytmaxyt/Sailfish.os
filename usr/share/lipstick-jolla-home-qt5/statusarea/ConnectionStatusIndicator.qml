...
Item {
    ...
    Timer {
        ...
        readonly property string iconId: networkManager.connectingWifi
                                         ? ("icon-status-wlan-connecting-" + iconLevel)
                                         : (actualLevel >= 0)
                                           ? ("icon-status-wlan-" + actualLevel) : ""
        readonly property int actualLevel: {
            var wifi = networkManager.connectedWifi
            if (wifi) {
        ...
        interval: 250
        running: connectionStatusIndicator.parent.visible && networkManager.connectingWifi
    ...
    property string _wlanIconId: {
        ...
        if (networkManager.connectedWifi || networkManager.connectingWifi)
    ...
    NetworkTrafficMonitor {
        ...
        polling: networkManager.available
