...
AttachmentDelegate {
    ...
    source: avatarUrl != "" ? avatarUrl
                            : (isThumbnail ? messagePart.path : "")
    fillMode: showFullImage ? Image.PreserveAspectFit : Image.PreserveAspectCrop
    height: showFullImage && isLandscape ? implicitHeight * width / implicitWidth
                                         : width
    ...
    BackgroundItem {
        ...
        onClicked: {
            if (isThumbnail) {
                var page = pageStack.push("ImageView.qml",
                                          { 'source': delegate.source, 'messagePart': delegate.messagePart })
                page.copy.connect(function() {
                    var filename = "mms-" + time.getFullYear() + padNumber(time.getMonth() + 1) + padNumber(time.getDate())
                    var source = delegate.source + ""
                    if (source.indexOf("file://") == 0) {
                        source = source.substring(7)
                        source = source.split("/").map(decodeURIComponent).join("/")
                    }

                    var dest = FileUtils.saveFile(source, filename,
                                                  extensionForType(delegate.messagePart.contentType),
                ...
                var filename = delegate.messagePart.path
                filename = filename.substring(filename.lastIndexOf('/') + 1)
                var split = filename.lastIndexOf('.')
                var extension = ""

                if (split > 0) {
                    extension = filename.substring(split)
                    filename = filename.substring(0, split)
                }

                var dest = FileUtils.saveFile(delegate.messagePart.path, filename, extension,
                                              FileUtils.DownloadLocation, "MMS")

                var url = "file://" + dest.split("/").map(encodeURIComponent).join("/")
