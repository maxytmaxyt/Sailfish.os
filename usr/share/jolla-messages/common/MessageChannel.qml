            ...
            groupManager.createOutgoingMessageEvent(groupId, localUid, remoteUids, text, function(broadcastEventId) {
                ...
                for (var i = 0; i < channels.length; i++) {
                    ...
                    var fn = function() {
                        ...
                        groupManager.createOutgoingMessageEvent(-1, channel.localUid, channel.remoteUid, text,
                                                                function(eventId) {
        ...
        if (event.eventType === CommHistory.MMSEvent) {
            ...
            for (var i = 0; i < channels.length; i++) {
                ...
                if (channels[i].localUid == event.localUid &&
                    groupManager.uidPairsMatch(event.localUid, event.remoteUid,
                                               channels[i].localUid, channels[i].remoteUid)) {
