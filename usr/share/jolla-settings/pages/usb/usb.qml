...
Page {
    ...
    USBSettings {
        ...
        onTargetModeChanged: {
            delayActivationMessageTimer.text = modeText.text
    ...
    // there's ~always a charging mode but the device doesn't necessarily actually support charging.
    // just change the strings based on battery feature
    property string preparingChargingText: deviceInfo.hasFeature(DeviceInfo.FeatureBattery)
                                           ? //% "Preparing for charging only"
                                             qsTrId("settings_usb-la-preparing-charging")
                                           : //% "Preparing"
                                             qsTrId("settings_usb-la-preparing")
    property string chargingLabel: deviceInfo.hasFeature(DeviceInfo.FeatureBattery)
                                   ? //% "Currently charging only"
                                     qsTrId("settings_usb-la-charging")
                                   : //% "No mode active"
                                     qsTrId("settings_usb-la-no_usb_mode")

    readonly property var usbModes: [
        {
            mode: usbSettings.MODE_CHARGER,
            menuText: (deviceInfo.hasFeature(DeviceInfo.FeatureBattery)
                       ? //% "Charger"
                         qsTrId("settings_usb-me-charger")
                       : //: Usb mode menu item where it does nothing, no even battery to charge
                         //% "None"
                         qsTrId("settings_usb-me-none_usb_mode")),
            // Dedicated charger but net effect is charging only
            preparingLabel: preparingChargingText,
            currentLabel: chargingLabel
        },{
            mode: usbSettings.MODE_CHARGING,
            menuText: (deviceInfo.hasFeature(DeviceInfo.FeatureBattery)
                       ? //% "Charging only"
                         qsTrId("settings_usb-me-charging")
                       : qsTrId("settings_usb-me-none_usb_mode")),
            preparingLabel: preparingChargingText,
            currentLabel: chargingLabel
        },{
        ...
        Column {
            ...
            Label {
                ...
                text: {
                    ...
                    if (delayActivationMessageTimer.running) {
                        return delayActivationMessageTimer.text
                    }
            ...
            Label {
                ...
                text: {
                    ...
                    if (disabledModes.length > 0) {
                        ...
                        return qsTrId("settings_usb_la-modes-disabled-by-mdm").arg(
                                    aboutSettings.baseOperatingSystemName)
                                + disabledModes.join(Format.listSeparator)
                    }
                    return ""
                }
            }
            ...
            Label {
                ...
                visible: usbSettings.configMode !== usbSettings.MODE_ASK
                         ...
                         && usbSettings.currentMode !== usbSettings.MODE_CHARGER
