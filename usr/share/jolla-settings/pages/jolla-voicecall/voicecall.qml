...
ApplicationSettings {
    ...
    Column {
        ...
        ActivateSimCardView {
            ...
            visible: networkManager.offlineMode
            width: parent.width
            ...
            onActionClicked: {
                networkManager.offlineMode = false   // NOTE: also used to set modem.online=false
            }
        ...
        ExpandingSectionGroup {
            ...
            Repeater {
                ...
                delegate: ExpandingSection {
                    ...
                    // hide section header if only one simcard.
                    buttonHeight: (title.length && simCardSettingsRepeater.count > 1) ? Theme.itemSizeMedium : 0
                    title: sailfishSimManager.ready
                           ? sailfishSimManager.simNames[sailfishSimManager.indexOfModem(modelData)] : ""
        ...
        Column {
            ...
            parent: simCardSettingsRepeater.count > 1 ? upperSimIndependentSettingsContainer
                                                      : lowerSimIndependentSettingsContainer
            width: parent.width
            ...
            BackgroundItem {
                ...
                Column {
                    ...
                    Label {
                        ...
                        //% "You can set up to five quick reply messages that you can send after "
            ...
            Column {
                ...
                SectionHeader {
                    //% "Blocked numbers"
                    text: qsTrId("settings_phone-la-blocked_numbers")
                }
                Repeater {
                    model: BlockedNumberModel {
                        id: blockedNumbers
                    }
                    delegate: ListItem {
                        Label {
                        ...
                        menu: Component {
                            ContextMenu {
                                MenuItem {
                                    //% "Remove"
                                    text: qsTrId("settings_phone-remove_blocked_number")
                                    onDelayedClick: blockedNumbers.removeItem(model.index)
                                }
                            }
                        }
                    }
                ...
                TextField {
                    property bool nonEmpty: text.trim().length > 0

                    //% "Add number"
                    label: qsTrId("settings_phone-add_number_to_block")
                    inputMethodHints: Qt.ImhDialableCharactersOnly

                    EnterKey.iconSource: nonEmpty ? "image://theme/icon-m-enter"
                                                  : "image://theme/icon-m-enter-close"
                    EnterKey.onClicked: {
                        var added = false
                        if (nonEmpty) {
                            if (blockedNumbers.containsNumber(text)) {
                                //% "Blocked numbers already contain %1"
                                Notices.show(qsTrId("settings_phone-blocked_numbers_already_contain").arg(text))
                            } else {
                                blockedNumbers.appendNumber(text)
                                added = true
                            }
                        }
                        ...
                        if (added)
                            text = "" // clear late to avoid flashing enter key close
                    }
                }
            }
        }
