    ...
    SilicaFlickable {
        ...
        Column {
            ...
            Column {
                ...
                opacity: enabled ? (content.enabled ? 1.0 : Theme.opacityLow)
                                 : 0.0
            ...
            Item {
                // This wrapper item always stays visible in the parent column, preventing a visual jump
                // to the section header below, caused by un-animated spacer item insertion/removal.
                width: parent.width
                height: Math.max(1, hotspotClientsItem.height)

                BackgroundItem {
                    id: hotspotClientsItem

                    clip: true
                    width: parent.width
                    height: wifiTethering.active ? Theme.itemSizeMedium : 0
                    visible: height > 0
                    ...
                    Label {
                        ...
                        text: networkManager.wifiClientCount === 0
                                ? //% "No hotspot clients"
                                  qsTrId("settings_network-la-hotspot-no-clients")
                                : //% "%n hotspot client(s)"
                                  qsTrId("settings_network-la-hotspot-n-clients", networkManager.wifiClientCount)
                    }

                    onClicked: pageStack.push(Qt.resolvedUrl("TetheringClientsPage.qml"), { technology: "wifi" })
                }
            }
