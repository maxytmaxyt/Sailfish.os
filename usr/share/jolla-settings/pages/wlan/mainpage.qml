...
Page {
    ...
    onStatusChanged: {
        if (status == PageStatus.Active) {
            ...
            if (showAddNetworkDialog) {
                ...
                var dialog = pageStack.push(Qt.resolvedUrl("AddNetworkDialog.qml"),
    ...
    SilicaListView {
        ...
        PullDownMenu {
            ...
            MenuItem {
                ...
                enabled: wifiTechnology.powered
        ...
        header: Column {
            ...
            ListItem {
                ...
                visible: wifiTechnology.available
                ...
                IconTextSwitch {
                    ...
                    checked: wifiTechnology.available && wifiTechnology.powered && !wifiTechnology.tethering
                    //% "WLAN"
                    text: qsTrId("settings_network-la-wlan")
                    enabled: AccessPolicy.wlanToggleEnabled
                             && (!wifiTechnology.tethering || AccessPolicy.internetSharingEnabled)
                    description: {
        ...
        model: wifiTechnology.available ? savedNetworks : null

        delegate: WlanItem { width: parent.width }

        // This is shown when connman is completely broken or there just is no wlan adapter
        Component {
            id: errorPlaceholderComponent

            ViewPlaceholder {
                opacity: wifiTechnology.available || !pageReady ? 0 : 1
                visible: opacity > 0.0
                //% "WLAN adapter not available"
                text: qsTrId("settings_network-la-wlan_unavailable")
        ...
        ViewPlaceholder {
            ...
            enabled: wifiTechnology.available && listView.count == 0 && connectMenuItem.enabled && !startupTimer.running
        }
    ...
    Timer {
        ...
        running: ((mainPage.status === PageStatus.Activating || mainPage.status === PageStatus.Active)
                  && wifiTechnology.available && wifiTechnology.powered
                  && Qt.application.state === Qt.ApplicationActive)
        repeat: true
        triggeredOnStart: true
        onTriggered: {
            if (!mainPage.suppressScan && !wifiTechnology.tethering) {
                wifiTechnology.scan()
            }
        }
    }
    ...
    NetworkTechnology {
        ...
        function maybeCreateErrorPlaceHolder() {
            if (!wifiTechnology.available && !_errorPlaceholder)
