...
Page {
    ...
    // When the network is connected show the set mDNS, otherwise show the
    // configured value that is not yet set for the network.
    property bool mDNSValue: network.connected ? network.mDNS : network.mDNSConfiguration
    property bool networkIsWPA3: network.securityType === NetworkService.SecuritySAE
    ...
    SilicaFlickable {
        ...
        Column {
            ...
            SectionHeader {
                text: {
                    switch (network.serviceState) {
                    case NetworkService.OnlineState:
                        //% "Connected"
                        return qsTrId("settings_network-la-connected_state")
                    case NetworkService.ReadyState:
            ...
            SectionHeader {
                //% "Configuration"
                text: qsTrId("settings_network-la-configuration")
            }

            DetailItem {
                //% "mDNS"
                label: qsTrId("settings_network-la-mdns")
                value: mDNSValue
                        ? qsTrId("settings_network-la-network_detail_enabled")
                        : qsTrId("settings_network-la-network_detail_disabled")
            }

            DetailItem {
                visible: networkIsWPA3

                //% "WPA3 SAE Check MFP"
                label: qsTrId("settings_network-la-wlan-sae_check_mfp")
                value: network.wpa3SaeCheckMfp
                        ? qsTrId("settings_network-la-network_detail_enabled")
                        : qsTrId("settings_network-la-network_detail_disabled")
            }
            ...
            DetailItem {
                ...
                //% "WPA3 SAE PWE"
                label: qsTrId("settings_network-la-wpa3_sae_pwe")
                value: WlanUtils.getSaePweDisplayString(network.wpa3SaePwe)
            }
        }
