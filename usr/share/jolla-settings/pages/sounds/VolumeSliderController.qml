...
Item {
    ...
    function updateSliderValue() {
        ...
        slider.value = volumeControlActive ? volumeControl.volume * 10
                                           : (profileControl.profile == "silent") ? 0
        ...
        PropertyChanges {
            ...
            onDownChanged: {
                if (volumeControlActive && sliderProperties.restrictedMaxDragXBinding) {
                    slider.drag.maximumX = Qt.binding(function() {
                        return sliderProperties.maxDragX - sliderProperties.stepWidth
                                * (volumeControl.maximumVolume - volumeControl.currentMax)
                    })
            ...
            onValueChanged: {
                if (!root.externalChange) {
                    // don't update slider until new values of ringVolume + profile are both known
                    root.propertiesUpdating = true
    ...
    SliderValueLabel {
        ...
        //% "%1%"
        text: slider.value > 0 ? qsTrId("settings_sounds-la-percentage_format")
                                 .arg(volumeControlActive ? scaledVolume : slider.value)
                               : ""
