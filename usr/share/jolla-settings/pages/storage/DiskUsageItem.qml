...
ListItem {
    ...
    readonly property bool errorState: !model.isSupportedFileSystemType
                                       && !model.isCryptoDevice
                                       && !diskUsageArea.busy
    ...
    onPartitionStatusChanged: {
        if (model.status == PartitionModel.Formatting)
            formattingInfo.start()
    }

    onClicked: {
        if (model.status == PartitionModel.Mounted && !removable) {
            pageStack.animatorPush('DiskUsagePage.qml',
                                   { parentPage: pageStack.currentPage, partition: model, title: getTitle() })
        }
    }
    ...
    Column {
        ...
        DiskUsageCircle {
            ...
            readonly property bool sizeDataAvailable: model.bytesTotal > 0

            property real animationValue: targetValue(0)

            function usedSpace() { return model.bytesTotal - model.bytesFree }

            function targetValue(base) {
                return base + (model.bytesTotal > 0 && model.bytesFree >= 0
                               ? (usedSpace() / model.bytesTotal) : 0)
            }

            function setupAnimationValue() {
                var base = Math.floor(animationValue)

                if (alternateColors) {
                    // Do not stop the animation when alternateColors is true.
                    base += 1
                } else if (model.status !== PartitionModel.Mounted
                           || value > (usedSpace() / model.bytesTotal)) {
            ...
            onSizeDataAvailableChanged: {
                if (sizeDataAvailable && !busy) {
                    setupAnimationValue()
                    // enable smoothing once we've gotten size data. try to ensure it happens only
                    // after initial value has been set
                    behaviorTimer.start()
                }
            }
            ...
            onBusyChanged: {
                if (busy) {
                    ...
                    // 24h hours, -> 43200, alternateColors false
                    animationValue = 30.0 * 60.0 * 24.0
                } else if (model.status !== PartitionModel.Mounted || sizeDataAvailable) {
                    setupAnimationValue()
                }
            }
            ...
            Timer {
                ...
                interval: 1
                onTriggered: usageBehavior.enabled = true
            }
            ...
            Behavior on animationValue {
                ...
                SmoothedAnimation {
                    duration: -1
                    velocity: circle.busy ? 0.5 : 1.5  // 0.5 = 0 -> 1 in two seconds.
        ...
        Column {
            ...
            Label {
                visible: diskUsageArea.removable
                         && model.filesystemType !== ""
                         && model.status != PartitionModel.Formatting
                width: contentWidth > _maxWidth ? _maxWidth : implicitWidth
                horizontalAlignment: contentWidth > _maxWidth ? Text.AlignLeft : Text.AlignHCenter
                truncationMode: TruncationMode.Fade
                anchors.horizontalCenter: parent.horizontalCenter
    ...
    DiskUsageLabelGroup {
        ...
        topLabelText: enabled && model.devicePath !== "" && model.bytesAvailable >= 0
                      ? Format.formatFileSize(model.bytesAvailable) : ""
        bottomLabelText: {
            if (!enabled) {
                return ""
            } else {
