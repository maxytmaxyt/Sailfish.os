    ...
    SilicaListView {
        ...
        PullDownMenu {
            ...
            MenuItem {
                ...
                //% "User data"
                onClicked: pageStack.animatorPush("DirectoryPage.qml",
                                                  { initialPath: diskUsagePage.path,
                                                      title: qsTrId("settings_storage-he-user_data") })
        ...
        delegate: DiskUsageListItem {
            ...
            onClicked: {
                if (dBusService) {
                    ...
                    dbusInterface.call(dBusMethod, dBusArgument,
                                       function() { /* success */ },
                                       function() {
                                           console.log("failed to call app for disk usage content type, opening file path")
                                           openPath()
                                       })
                } else {
                    openPath()
            ...
            function openPath() {
                if (pathAllowed) {
                    pageStack.animatorPush("DirectoryPage.qml",
                                           { initialPath: path, title: path.replace(StandardPaths.home + "/", "") })
                }
            }
        }
