        ...
        Column {
                ...
                text: {
                    if (partition.drive.connectionBus === PartitionModel.USB) {
                        return partition.filesystemType !== ""
                                ? //: %1 is the name of the incompatible file system, e.g. 'exFAT'.
                                  //: %2 is the name of the compatible file system, e.g. 'ext4'.
                                  //% "This USB storage uses an incompatible file system (using %1). "
                                  //% "To be able to use the card, formatting to a compatible file system (%2) is needed."
                                ...
                                : //: %1 is the name of the compatible file system, e.g. 'ext4'.
                                  //% "This USB storage uses an unknown incompatible file system. "
                                  //% "To be able to use the card, formatting to a compatible file system (%1) is needed."
                                  qsTrId("settings_storage-la-unknown_incompatible_usb_storage_filesystem").arg(root._targetFilesystem)
                    } else {
                        return partition.filesystemType !== ""
                                ? //: %1 is the name of the incompatible file system, e.g. 'exFAT'.
                                  //: %2 is the name of the compatible file system, e.g. 'ext4'.
                                  //% "This memory card uses an incompatible file system (using %1). "
                                  //% "To be able to use the card, formatting to a compatible file system (%2) is needed."
                                  qsTrId("settings_storage-la-incompatible_memory_card_filesystem").arg(partition.filesystemType).arg(root._targetFilesystem)
                                : //: %1 is the name of the compatible file system, e.g. 'ext4'.
                                  //% "This memory card uses an unknown incompatible file system. "
                                  //% "To be able to use the card, formatting to a compatible file system (%1) is needed."
            ...
            StorageLabel {
                text: partition.drive.connectionBus === PartitionModel.USB
                      ? //% "Please make a backup of the USB storage contents before formatting. "
                        //% "All data on the card will be destroyed."
                        qsTrId("settings_storage-la-usb_storage_backup_warning")
                      : //% "Please make a backup of the memory card contents before formatting. "
                        //% "All data on the card will be destroyed."
            ...
            TextField {
                ...
                EnterKey.iconSource: password.enabled ? "image://theme/icon-m-enter-next"
                ...
                StorageLabel {
                    ...
                    text: partition.drive.connectionBus === PartitionModel.USB
                          ? //: Shown when "Encrypt" switch is enabled on the format page for USB storage.
                            //% "The encrypted USB storage is meant to be used with your device only. "
                            //% "There may be compatibility issues with other systems."
                          ...
                          : //: Shown when "Encrypt" switch is enabled on the format page for memory card.
                            //% "The encrypted card is meant to be used with your device only. "
                            //% "There may be compatibility issues with other systems."
            ...
            Button {
                ...
                onClicked: {
                    ...
                    if (encryptSwitch.checked && partition.status === PartitionModel.Mounted) {
                        ...
                        _busy = true
                        var devicePath = partition.cryptoBackingDevicePath ? partition.cryptoBackingDevicePath
                        ...
                        model.format(devicePath, {
                                         ...
                                         "mkfs-args": root._targetFilesystem == "ext4" ? "-m 0" : ""
                                     })
                    }
                    pageStack.pop()
                }
