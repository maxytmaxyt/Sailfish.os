...
Item {
    ...
    function openSandboxed(name, treeItem, icon, filePath, organizationName, applicationName) {
        var page
        ...
        pageStack.animatorPush(page,
                               {
                                   ...
                                   "_desktopFile": filePath,
                                   "sailjailOrganizationName": organizationName,
                                   "sailjailApplicationName": applicationName
                               })
    }
    ...
    ApplicationsGridView {
        ...
        delegate: Item {
            ...
            LauncherGridItem {
                ...
                property bool configurable: !!model.section
                ...
                onClicked: {
                    if (model.sandboxed) {
                        root.openSandboxed(text, model.section, model.iconId, model.filePath,
                                           model.organizationName, model.applicationName)
                    } else if (configurable) {
                        root.openSettings(text, model.section, model.iconId)
                    } else if (model.androidApp) {
                        root.openAndroidSettings(text, model.section, model.iconId,
                                                 model.androidAppPkg, model.androidAppVer)
                    }
                }
            }
        }
