...
Page {
        ...
        RemorsePopup {
            ...
            onTriggered: {
                if (resetType === "cellular") {
                    ...
                    networkManager.resetCountersForType(resetType)
                    usageCounter.wifiData = {}
                    wlanResetTime.value = pendingResetTime.toISOString()
                }
    ...
    NetworkCounter {
        ...
        property var pendingUsage: filterUnneededUsage(networkManager.savedServicesList())
        ...
        function resetServiceCounter(servicePath) {
            ...
            if (index >= 0)
                cellularResetTimes.setValue(servicePath.substr(index + 1), pendingResetTime.toISOString())
        }
    ...
    // Define as property to avoid id that is same as the MobileDataCounterGroup property
    property var networkManager: NetworkManager {
        ...
        function updateCellularServices() {
            var newCellularServices = servicesList("cellular")
        ...
        onServicesChanged: {
            ...
            if (usageCounter.running && usageCounter.pendingUsage.length > 0) {
                ...
                if (pendingServices.length > 0) {
                    ...
                    usageCounter.pendingUsage = filterUnneededUsage(savedServicesList())
                    usageCounter.restart()
                }
            }
        }

        onAvailableChanged: updateCellularServices()
