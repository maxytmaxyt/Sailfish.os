...
Page {
        ...
        if (status === PageStatus.Active) {
            if (importPath) {
                ...
                if (mimeHandlers && mimeHandlers[importMime]) {
                    VpnTypes.importFile(pageStack, root, importPath,
                                        mimeHandlers[importMime].vpnType, mimeHandlers[importMime].parser)
                    importPath = ''
                    importPath = ''
                    pendingMime = false
                } else {
    ...
    SilicaListView {
        ...
        delegate: VpnItem {
            ...
            isDefault: defaultVpnId.value === connection.path
            onToggleDefault: defaultVpnId.value = isDefault ? "" : connection.path
        }
