...
SettingsToggle {
    ...
    onToggled: {
        ...
        if (checked) {
            for (i = 0; i < SettingsVpnModel.count; ++i) {
                conn = SettingsVpnModel.get(i)
                conn.autoConnect = false
            }
        } else if (SettingsVpnModel.count === 1) {
            SettingsVpnModel.get(0).autoConnect = true
        // If there is a value set for defaultVPNId automatically connect that VPN
        } else if (defaultVpnId.value !== "") {
            var found = false

            for (i = 0; i < SettingsVpnModel.count; ++i) {
                conn = SettingsVpnModel.get(i)

                if (defaultVpnId.value === conn.path) {
            ...
            // Stale configuration value, show settings
            if (!found)
    ...
    ConfigurationValue {
        id: defaultVpnId
        key: "/apps/jolla-settings/defaultVpnId"
    }
}
