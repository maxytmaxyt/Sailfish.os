...
ListItem {
    ...
    readonly property bool connected: connection && connection.state === VpnConnection.Ready
    ...
    menu: Component {
        ContextMenu {
            ...
            MenuItem {
                ...
                onClicked: {
                    pageStack.animatorPush(VpnTypes.detailsPagePath(connection.type),
                                           { connection: SettingsVpnModel.connectionSettings(connection.path) })
                }
            }
            MenuItem {
                //% "Unset as default"
                text: isDefault ? qsTrId("settings_network-me-unset_as_default") :
                          //% "Set as default"
                          qsTrId("settings_network-me-set_as_default")
                // Use path as it is an unique value, name is not.
                onDelayedClick: toggleDefault()
            }
        }
    }
    ...
    IconTextSwitch {
        ...
        icon.source: connection && isDefault ? "image://theme/icon-m-vpn" : ""
        visible: !textNoSwitch.visible
        automaticCheck: false
        checked: connection && connection.autoConnect
        highlighted: root.highlighted
        busy: connection && (connection.state === VpnConnection.Configuration
                             || connection.state === VpnConnection.Association
                             || connection.state === VpnConnection.Disconnect)
    ...
    TextNoSwitch {
        ...
        onPressAndHold: root.openMenu()
        onClicked: pageStack.animatorPush(VpnTypes.detailsPagePath(connection.type),
                                          { connection: SettingsVpnModel.connectionSettings(connection.path) })
    }
}
