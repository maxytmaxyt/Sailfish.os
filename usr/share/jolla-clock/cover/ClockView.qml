...
Item {
    ...
    ShaderEffect {
        ...
        property int stopWatchDuration: (_stopwatchMode && stopwatch) ? stopwatch.totalTime : 0
        onStopWatchDurationChanged: _updateTimerVisualization()

        property date startTime: {
            if (timerClock && timerClock.alarm)
                return time
            else
                return new Date
        }
        ...
        function _updateTimerVisualization() {
            var duration = _stopwatchMode ? stopWatchDuration : (endTime - startTime)
            var end
