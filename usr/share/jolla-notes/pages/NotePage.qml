    ...
    SilicaFlickable {
        ...
        PullDownMenu {
            ...
            MenuItem {
                ...
                onClicked: {
                    var fileName = page.noteFileName(noteview.text) + (transferAsVNoteConfig.value == true
                                                                       ? ".vnt" : ".txt")
                    var mimeType = transferAsVNoteConfig.value == true ? "text/x-vnote" : "text/plain"
                    // vnoteConverter is a global installed by notes.cpp
                    var noteText = transferAsVNoteConfig.value == true ? vnoteConverter.vNote(textArea.text)
                                                                       : textArea.text
                    var content = {
                        "name": fileName,
                        "data": noteText,
