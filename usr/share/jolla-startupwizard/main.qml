...
ApplicationWindow {
    ...
    property Component _firstPageAfterPinQuery: {
        ...
        } else if (deviceLockSettings.homeEncrypted
                   || DeviceLock.state != DeviceLock.Unlocked
                   || !DeviceLock.enabled) {
    ...
    property Component _pageAfterAccountSetup: fingerprintSettings.hasSensor
                                               ...
                                               : _pageAfterFingerPrint ? _pageAfterFingerPrint : tutorialMainComponent

    readonly property Component _pageAfterFingerPrint: {
        var pageAfterFingerPrint = Qt.createComponent(Qt.resolvedUrl("extensions/PageAfterFingerPrint.qml"))
        if (pageAfterFingerPrint.status != Component.Ready) {
            pageAfterFingerPrint = null
        }
        return pageAfterFingerPrint
    }
    ...
    Connections {
        ...
        onCurrentPageChanged: {
            ...
            if (!root._enteredTutorial && pageStack.currentPage.hasOwnProperty("allowSystemGesturesBetweenLessons")) {
                ...
                // tutorial is portrait only for now
                root.allowedOrientations = Orientation.Portrait
                tutorialExitConf()
            }
        }
    }
    ...
    Component {
        ...
        NetworkCheckDialog {
            readonly property bool dateTimeSettingsEnabled: AccessPolicy.dateTimeSettingsEnabled
                                                            && AccessControl.hasGroup(AccessControl.RealUid,
                                                                                      "sailfish-datetime")
    ...
    Component {
        ...
        FingerEnrollmentWelcomeDialog {
            header.cancelText: ""
            canSkip: true
            destination: _pageAfterFingerPrint ? _pageAfterFingerPrint : tutorialMainComponent
